...
describe('タイミング方式の進行', () => {
  ...
  // 1ターン目に テラスタル を きかれるので、ことわってから すすめる
  const start = () =>
    apply(
...
describe('メガわざ は ちからを つかいきる（docs/SPEC.md §3.6）', () => {
  ...
  function megaReady() {
    let state = createBattle(
      [makePick({ canMegaEvolve: true, energy: 200 })],
...
describe('テラスタル（docs/SPEC.md §3.11）', () => {
  const teraSettings = makeSettings({ attackStyle: 'timing', fatigueEnabled: false });

  /** 3vs3 で バトルを はじめる。1ターン目から テラスタル を きかれる */
  function start(size = 3) {
    const team = () => Array.from({ length: size }, () => makePick({ energy: 900 }));
  ...
  it('えらんだ子が テラスタル して、チームの 1回を つかいきる', () => {
    ...
    expect(state.teraUsed.p1).toBe(false);
  ...
  it('テラスタルわざ は テラスタル中の子だけ えらべる', () => {
    ...
    expect(other.phase).toBe('chooseMove');
  ...
  it('テラスタルわざ は あいて ぜんいん に あたり、ダメージは 等分', () => {
    ...
    for (const hit of result.hits) expect(hit.damage).toBe(150);
    // ぜんいん の たいりょくが へっている
    for (const pokemon of state.teams.p2) expect(pokemon.hp).toBe(pokemon.maxHp - 150);
  });

  it('あいてが 1体だけなら、つよいわざ と おなじ ダメージ（ちらすより あつめたほうが 大きい）', () => {
    let state = createBattle(
      [makePick(), makePick()],
      [makePick({ energy: 900 })],
      'p1',
    ...
    // 450 ÷ 1.5 = 300。つよいわざ の ぴったり と おなじ
