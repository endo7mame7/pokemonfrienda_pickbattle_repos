...
const dice = (rolls: number[]): AttackInput => ({ style: 'dice', rolls });
...
describe('calcDamage（docs/SPEC.md §3.4 の実例）', () => {
  ...
  it('ふつう: 出目3 → 60', () => {
    const result = calcDamage(attacker, normalTarget, dice([3]), settings);
  ...
  it('ばつぐん: 出目3 → 60 + 20 = 80', () => {
    const result = calcDamage(attacker, weakTarget, dice([3]), settings);
  ...
  it('つかれてる: 出目3 → 60 ÷ 2 = 30', () => {
    ...
    const result = calcDamage(tired, normalTarget, dice([3]), settings);
  ...
  it('つかれてる + ばつぐん: (60 + 20) ÷ 2 = 40', () => {
    const tired = makePokemon({ type: 'ほのお', tired: true });
    expect(calcDamage(tired, weakTarget, dice([3]), settings).damage).toBe(40);
  });

  it('メガシンカ中はサイコロ2個ぶん: 出目 4+5 → 180', () => {
    expect(calcDamage(attacker, normalTarget, dice([4, 5]), settings).damage).toBe(180);
  });

  it('メガシンカ中 + ばつぐん: 180 + 20 = 200', () => {
    expect(calcDamage(attacker, weakTarget, dice([4, 5]), settings).damage).toBe(200);
  });
});
...
describe('calcDamage の設定', () => {
  it('ダメージばいりつを変えられる', () => {
    ...
    expect(calcDamage(attacker, target, dice([3]), makeSettings({ damageMultiplier: 10 })).damage).toBe(30);
    expect(calcDamage(attacker, target, dice([3]), makeSettings({ damageMultiplier: 30 })).damage).toBe(90);
  });
  ...
  it('ばつぐんボーナスを変えられる', () => {
    ...
    for (const bonus of [0, 10, 20, 40, 60] as const) {
      ...
      expect(calcDamage(attacker, target, dice([3]), settings).damage).toBe(60 + bonus);
    }
  });
  ...
  it('つかれルールをオフにすると、つかれていても半分にならない', () => {
    ...
    const result = calcDamage(tired, makePokemon(), dice([3]), settings);
    expect(result.damage).toBe(60);
    expect(result.isTired).toBe(false);
  });
      ...
      for (const superEffectiveBonus of bonuses) {
        for (const tired of [false, true]) {
          for (const rolls of [[1], [6], [1, 1], [6, 6], [3, 4]]) {
            ...
            const target = makePokemon({ type: 'くさ' });
            expect(calcDamage(attacker, target, dice(rolls), settings).damage % 10).toBe(0);
          }
        }
      }
    }
  ...
  it('ダメージが0になることはない（園児が「なにも できなかった」体験をしない）', () => {
    ...
    expect(calcDamage(attacker, makePokemon(), dice([1]), settings).damage).toBeGreaterThan(0);
  });
});
